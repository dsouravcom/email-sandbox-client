...
export class VerifyEmail {
  ...
  /** Set by the register/login pages; missing after a page reload, so the email stays editable. */
    ...
    },
    {
      submission: {
        action: async (field) => {
          ...
          try {
            await this.authStore.verifyEmail(field().value());
            const provisioning = this.authStore.consumeLastProvisioning();
            if (provisioning) {
  ...
  constructor() {
    // Arriving from sign-up or sign-in means a code was just sent.
  ...
  protected async resend(): Promise<void> {
    ...
    this.errorMessage.set(null);
    try {
      const { message } = await firstValueFrom(this.authApi.resendVerification(emailField.value()));
